#!/usr/bin/env python3
"""
scripts/build_1970s_peak_ratings.py — derive per-player peak 1970s 2K overalls.

Same methodology as scripts/build_1960s_peak_ratings.py: NBA 2K didn't exist
before 1999, so 1970s players have no in-era rating either. This reads 2K's
classic/all-time roster ratings (Classic Teams, All-Time Teams, All-Decade
Teams) and keeps each player's single highest ("peak") overall across every
such appearance.

Source: data/nba2k_1970s_sources/2kratings_websearch_compiled.json, compiled
via targeted web search against 2kratings.com (same access constraints as the
1960s file — see data/README.md for the full Cloudflare/proxy-allowlist
caveat). Every row is a real number attributed to a specific 2kratings.com
roster page, never a model guess.

Usage:
    python3 scripts/build_1970s_peak_ratings.py
"""
import json
import os

ROOT = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..")
SRC_DIR = os.path.join(ROOT, "data", "nba2k_1970s_sources")
OUT_PATH = os.path.join(ROOT, "data", "nba2k_1970s_peak_ratings.json")
