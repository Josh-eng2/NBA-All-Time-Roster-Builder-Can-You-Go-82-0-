...
| `leaderboard-wire.test.mjs` | submitted documents stay inside the deployed Firestore rule ranges — an out-of-range field loses the whole submission — and the builders' key sets match the `hasOnly()` lists read back out of `firestore.rules`, in both directions; the optional SDK modules (analytics, App Check, performance) can each be blocked without taking the leaderboard down; `measure()` stays invisible when the Performance SDK never loads |
...
`browser-playtest-regressions.mjs` uses the same external Playwright settings and starts its own local server. It completes drafts at 390×844, 844×390 and 320×568; checks readable phone text and tap targets; exercises same-tab and cross-tab account handoffs; and upgrades two open tabs from both v34 and v35. Run it with `node tests/browser-playtest-regressions.mjs`. Its pinned old commits require full Git history (`git fetch --unshallow` for a shallow clone); `MIGRATION_REFS` can supply other comma-separated old commit refs. It never contacts live accounts or leaderboards.

`account-deletion.test.mjs` runs the real modal and authentication wrapper against a fake SDK, covering identity changes before confirmation, during cloud deletion and during authentication deletion. The transport suite separately verifies that queued cloud deletion refuses a changed session. Both run in the normal dependency-free suite.
